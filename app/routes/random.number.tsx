...
export function meta() {
	return [
		...
		{
			name: "description",
			content: "เครื่องมือนี้ช่วยให้คุณสามารถสร้างตัวเลขสุ่มระหว่างช่วงที่กำหนดได้อย่างง่ายดาย. ใช้งานง่ายและรวดเร็ว.",
		},
	];
}
...
export default function RandomNumber() {
  ...
  const startAnimation = (min: number, max: number) => {
    setIsAnimating(true);
    const interval = setInterval(() => setDisplayNumber(getRandomNumber(min, max)), ANIMATION.interval);
  ...
  const getDisplayPlaceholder = () =>
    ...
    const min = range ? range.min : Number.parseInt(fromRef.current?.value || String(DEFAULT_RANGE.from));
    const max = range ? range.max : Number.parseInt(toRef.current?.value || String(DEFAULT_RANGE.to));
      ...
      >
        {DIGIT_OPTIONS.map(({ value, label }) => (
          <div key={value} className="flex items-center gap-2">
			...
			{selectedOption === "custom" && <RangeInputs fromRef={fromRef} toRef={toRef} />}
